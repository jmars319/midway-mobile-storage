...
// start a session to support form flash for non-AJAX submissions
...
// certifications (optional checkbox array)
$certifications = is_array($_POST['certifications'] ?? null) ? $_POST['certifications'] : [];
...
// Resume upload handling: place under uploads/resumes/, safe filename, limit 2MB
$resumeFileName = '';
if (!empty($_FILES['resume']) && is_uploaded_file($_FILES['resume']['tmp_name'])) {
    ...
    $uExt = strtolower(pathinfo($resume['name'], PATHINFO_EXTENSION));
    $finfoOk = false;
    ...
    if ($finfo) {
        ...
        if (in_array($mime, $allowedMimes)) $finfoOk = true;
    }
    if ($resume['size'] <= $maxSize && in_array($uExt, $allowedExt) && $finfoOk) {
        // store under private_data/resumes (outside webroot)
        $resumesDir = __DIR__ . '/../private_data/resumes';
        if (!is_dir($resumesDir)) @mkdir($resumesDir, 0755, true);
        $safeBase = preg_replace('/[^a-z0-9\-_]/i', '_', pathinfo($resume['name'], PATHINFO_FILENAME));
        $ts = function_exists('eastern_now') ? eastern_now('Ymd_His') : date('Ymd_His');
        $newName = $safeBase . '_' . $ts . '.' . $uExt;
        $dest = $resumesDir . '/' . $newName;
        if (@move_uploaded_file($resume['tmp_name'], $dest)) {
            // store only filename in entry. The admin download endpoint will read from private_data/resumes
            $resumeFileName = $newName;
        } else {
            $errors[] = 'Failed to save uploaded resume file';
        }
    } else {
        // A file was uploaded but failed validation -> block submission
        $errors[] = 'Resume upload failed: file must be PDF or Word (.pdf, .doc, .docx) and under 2MB';
    }
}
...
if ($submission_type === 'application') {
    ...
    // Default minimum age for general roles
    $requiredMin = 18;
    // Drivers have a higher minimum age
    if (strtolower($position_desired) === 'driver') {
        $requiredMin = 21;
    }
    // Administrative or fabricator roles can be 18+
...
if (!empty($errors)) {
    if ($isAjax) { http_response_code(400); echo json_encode(['success' => false, 'errors' => $errors]); }
        ...
        // preserve entered values and errors so the public form can show them
        $_SESSION['form_flash'] = ['errors' => $errors, 'values' => $_POST];
...
$to = $GLOBALS['APPLICATIONS_TO'] ?? 'info@midwaymobilestorage.com';
$subject = 'Application: ' . $first_name . ' ' . $last_name . ' — ' . $position_desired;
$body = "Midway Mobile Storage - Job Application\n\n";
$body .= "Name: {$first_name} {$last_name}\nEmail: {$email}\nPhone: {$phone}\nAddress: {$address}\n\n";
$body .= "Position: {$position_desired}\nEmployment type: {$employment_type}\nAvailability: {$availability_text}\n\n";
$body .= "Experience: " . ($message ? $message : ($why_work_here ?: 'Not provided')) . "\n\n";
if (!empty($certifications)) {
    $body .= "Certifications: " . implode(', ', $certifications) . "\n\n";
}
if ($resumeFileName !== '') {
    $body .= "Resume: {$resumeFileName}\n\n";
}
